...
SharedPtr<TreeViewItem> createLabelAreaPage(ConstraintLayout* page)
{
    ...
    auto item = makeUIObject<TreeViewItem>(
        IconLabel::compactLayout(L"Label Area", nullptr, 1.0f, 11.0f, 23.0f));

    item->resize(item->width(), 40.0f);
    item->getContent<IconLabel>().lock()->label()->
        setTextFormat(D14_FONT(L"Default/Normal/14"));

    auto icon1 = loadBitmap(L"SideCategory/Light/LabelArea.png");
    auto icon2 = loadBitmap(L"SideCategory/Dark/LabelArea.png");
