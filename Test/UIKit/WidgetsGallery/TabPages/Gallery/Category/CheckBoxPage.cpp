...
SharedPtr<TreeViewItem> createCheckBoxPage(ConstraintLayout* page)
{
    ...
    auto item = makeUIObject<TreeViewItem>(
        IconLabel::compactLayout(L"Check Box", nullptr, 1.0f, 14.0f, 26.0f));

    item->resize(item->width(), 40.0f);
    item->getContent<IconLabel>().lock()->label()->
        setTextFormat(D14_FONT(L"Default/Normal/14"));

    auto icon1 = loadBitmap(L"SideCategory/Light/CheckBox.png");
    auto icon2 = loadBitmap(L"SideCategory/Dark/CheckBox.png");
