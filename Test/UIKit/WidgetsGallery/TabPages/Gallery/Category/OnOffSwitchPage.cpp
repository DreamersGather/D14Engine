...
SharedPtr<TreeViewItem> createOnOffSwitchPage(ConstraintLayout* page)
{
    ...
    auto item = makeUIObject<TreeViewItem>(
        IconLabel::compactLayout(L"On-Off Switch", nullptr, 1.0f, 9.0f, 21.0f));

    item->resize(item->width(), 40.0f);
    item->getContent<IconLabel>().lock()->label()->
        setTextFormat(D14_FONT(L"Default/Normal/14"));

    auto icon1 = loadBitmap(L"SideCategory/Light/OnOffSwitch.png");
    auto icon2 = loadBitmap(L"SideCategory/Dark/OnOffSwitch.png");
